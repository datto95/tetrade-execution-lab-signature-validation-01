...
contract VulnerableSignatureVault {
    ...
    function getDigest(address recipient, uint256 amount, uint256 deadline) public view returns (bytes32) {
        return keccak256(abi.encodePacked(address(this), recipient, amount, deadline));
    }

    function claim(address payable recipient, uint256 amount, uint256 deadline, bytes calldata signature) external {
        if (block.timestamp > deadline) revert AuthorizationExpired();

        bytes32 digest = getDigest(recipient, amount, deadline);
